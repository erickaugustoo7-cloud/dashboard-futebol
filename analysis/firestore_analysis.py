...
"""
Script de análise de dados de partidas de futebol.
Lê dados do Firestore, limpa e calcula estatísticas com Pandas.
...
def fetch_matches(
    ...
    for doc in docs:
        ...
        record = {
            ...
            "goals_raw":   goals_raw,  # lista de dicts — útil para análises pontuais
        }
        records.append(record)

    df = pd.DataFrame(records)

    # Converter Timestamp Firestore → datetime pandas
...
def clean_matches(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Estatísticas: nulo → 0 (API não retorna stats de jogos não encerrados)
    stat_cols = [
    ...
    # Placar: nulo → 0 (partidas agendadas ainda não têm placar)
    ...
    # Total de cartões e escanteios por partida
    ...
    # Virada de placar no segundo tempo
    df["comeback"] = (
...
def detect_outliers(df: pd.DataFrame, column: str) -> pd.DataFrame:
    """
    Detecta outliers em uma coluna numérica usando o método IQR.
    ...
    Returns:
        DataFrame contendo apenas as linhas consideradas outliers.
    """
...
def run_analysis(df: pd.DataFrame) -> dict:
    ...
    Returns:
        Dicionário com os principais indicadores.
    """
    print("\n" + "═" * 58)
    print("  📈  ANÁLISE DE PARTIDAS — BRASILEIRÃO")
    print("═" * 58)
    ...
    # Top mandantes por escanteios
    print(f"\n🏠 Top 5 mandantes (média de escanteios):")
    top_home_corners = (
        finished.groupby("home_team")["corners_home"]
        .agg(["mean", "count"])
        .query("count >= 3")          # Mínimo 3 partidas para ter representatividade
...
if __name__ == "__main__":
    ...
    # Exportar CSV (sem a coluna de lista bruta)
