...
export default function ConfrontoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/suggestions?date=${date?.substring(0, 10) ?? new Date().toISOString().split('T')[0]}`).then(r => r.json()),
  ...
  const dateLabel = date ? new Date(date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' }) : '';
