            ...
            {filtered.map((match: any) => {
              ...
              return (
                <div key={match.fixture_id} className={`match-card ${confClass}-confidence`}>
                  ...
                  <Link
                    href={`/confronto/${match.fixture_id}?home_id=${match.home_team_id}&away_id=${match.away_team_id}&home=${encodeURIComponent(match.home_team)}&away=${encodeURIComponent(match.away_team)}&league=${encodeURIComponent(match.league_name)}&date=${match.date || date}`}
                    style={{
                      display: 'block',
                      marginTop: '12px',
                      padding: '8px',
